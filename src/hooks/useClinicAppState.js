...
function useClinicAppState(navigate) {
  ...
  const stats = useMemo(
    () => [
      ...
      { label: "Registered Doctors", value: doctors.length, icon: UserCog },
      {
        ...
        value: cases.filter((c) => c.appointmentDate && c.status !== "closed")
          .length,
        icon: CalendarDays,
      },
      {
        label: "Pending Lab",
        value: cases.filter((c) => c.status === "lab").length,
        icon: FlaskConical,
      },
      {
        label: "Pending Billing",
        value: cases.filter((c) => c.status === "reception").length,
        icon: CircleDollarSign,
      },
      {
        label: "Pending Pharmacy",
        value: cases.filter((c) => c.status === "pharmacy").length,
        icon: Pill,
      },
    ],
  ...
  const updateCase = (id, patch) => {
    api(`/cases/${id}`, { method: "PATCH", body: JSON.stringify(patch) })
      .then((updated) => {
        setCases((prev) =>
          prev.map((c) => (c.id === id ? { ...updated, id: updated._id } : c)),
        );
      })
      .catch(() => {});
  };
  ...
  return {
    ...
    onApproveRequest: (requestId) =>
      api(`/admin/doctor-requests/${requestId}/approve`, {
        method: "POST",
      }).then(() => loadAll("admin")),
    onRejectRequest: (requestId) =>
      api(`/admin/doctor-requests/${requestId}`, { method: "DELETE" }).then(
        () => loadAll("admin"),
      ),
    onAddDoctor: (doctorAccount) =>
      api("/doctors", {
        method: "POST",
        body: JSON.stringify(doctorAccount),
      }).then(() => loadAll("admin")),
    onUpdateDoctor: (doctorId, patch) =>
      api(`/doctors/${doctorId}`, {
        method: "PATCH",
        body: JSON.stringify(patch),
      }).then(() => loadAll("admin")),
    onDeleteDoctor: (doctorId) =>
      api(`/doctors/${doctorId}`, { method: "DELETE" }).then(() => {
        // Remove the doctor from local state using both _id and id
        setUsers((prev) => {
          ...
          console.log("Deleted doctorId:", doctorId);
          console.log("Users before:", prev.length, "after:", filtered.length);
          return filtered;
        });
        // Still fetch fresh list to stay completely in sync
        return loadAll("admin");
      }),
    onAddLabTest: (payload) =>
      api("/catalog/lab-tests", {
        method: "POST",
    ...
    generateBill: (billData) =>
      api("/bills", { method: "POST", body: JSON.stringify(billData) }),
