...
function ReceptionDesk({
  ...
  const checkSlotAvailability = async () => {
    if (!form.doctorName || !form.appointmentDate || !form.appointmentTime)
      return true;
    const selectedDoctor = doctors.find((d) => d.name === form.doctorName);
    ...
    try {
      ...
      const isBooked = appointments.some(
        (apt) =>
          apt.time === form.appointmentTime && apt.status !== "cancelled",
      );
        ...
        try {
          ...
          const billData = {
            ...
            appointmentDetails: {
              ...
              day: appointmentDate.toLocaleDateString("en-US", {
                weekday: "long",
              }),
            },
  ...
  return (
    <section className="form-panel">
      ...
      <p>
        One user handles appointment booking, billing, and token issuance in a
      ...
      <div className="form-grid">
        ...
        />
        {checkingSlot && (
          <small className="muted">Checking slot availability...</small>
        )}
        <input
          placeholder="Reason"
          value={form.reason}
      ...
      <button type="button" onClick={addCase} disabled={checkingSlot}>
        {checkingSlot
          ...
          : "Create Appointment & Send to Doctor"}
      </button>
      ...
      <CaseTable
        cases={billingQueue}
        actions={(c) => {
          const doctorFee = c.billingPaid
            ? 0
            : Number(
                doctors.find(
                  (d) =>
                    d.id === (c.doctorId || c.doctorId?._id || c.doctorId?.id),
                )?.consultFee || 0,
              );
          const testsTotal = (c.recommendedTests || []).reduce(
            (acc, t) => acc + Number(t.price || 0),
            0,
          );
          const medsTotal = (c.prescriptions || []).reduce(
          ...
          const total =
            ...
            testsTotal +
            medsTotal +
            Number(extraLabFee[c.id] || 0) +
            Number(extraPharmFee[c.id] || 0);
          return (
            <>
              ...
              <p className="muted inline-text">
                Medicines total: PKR {medsTotal}
              </p>
              <input
                placeholder="Extra lab charges"
                value={extraLabFee[c.id] || ""}
                onChange={(e) =>
                  setExtraLabFee({ ...extraLabFee, [c.id]: e.target.value })
                }
              />
              <input
                placeholder="Extra pharmacy charges"
                value={extraPharmFee[c.id] || ""}
                onChange={(e) =>
                  setExtraPharmFee({ ...extraPharmFee, [c.id]: e.target.value })
                }
              />
              <button
                type="button"
                onClick={async () => {
                  ...
                  try {
                    ...
                    await onUpdate(c.id, {
                      ...
                      status: c.recommendedTests?.length
                        ? "lab"
                        : c.prescriptions?.length
                          ? "pharmacy"
                          : "closed",
                      labStatus: c.recommendedTests?.length
                        ? "pending"
                        : "not_required",
                      pharmacyStatus: c.prescriptions?.length
                        ? "pending"
                        : "not_required",
                    ...
                    // Generate the bill
                    const labTestsForBill = (c.recommendedTests || []).map(
                      (t) => ({
                        name: t.name,
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const token =
                      c.token || `TKN-${Date.now().toString().slice(-6)}`;
                    const labTestsForBill = (c.recommendedTests || []).map(
                      (t) => ({
                        name: t.name,
                        price: Number(t.price || 0),
      ...
      {consultationBill && (
        <div className="bill-container printable-bill receipt-card">
          <div className="bill-header">
            ...
            <button
              type="button"
              onClick={() => setConsultationBill(null)}
              className="close-btn"
            >
              ×
            </button>
          ...
          <div className="bill-details">
            <div className="bill-section">
              ...
              <p>
                <strong>Name:</strong> {consultationBill.patientName}
              </p>
            </div>
            ...
            <div className="bill-section">
              ...
              <p>
                <strong>Doctor:</strong> {consultationBill.doctorName}
              </p>
            </div>
            ...
            <div className="bill-section">
              ...
              {consultationBill.appointmentDetails && (
                <>
                  <p>
                    <strong>Date:</strong>{" "}
                    {consultationBill.appointmentDetails.date}
                  </p>
                  <p>
                    <strong>Time:</strong>{" "}
                    {consultationBill.appointmentDetails.time}
                  </p>
                  <p>
                    <strong>Day:</strong>{" "}
                    {consultationBill.appointmentDetails.day}
                  </p>
                </>
              )}
            </div>
            ...
            <div className="bill-section">
              ...
              <div className="bill-breakdown">
                ...
                <div className="bill-total">
                  ...
                  <span>
                    <strong>PKR {consultationBill.totalAmount}</strong>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bill-actions">
            <button
              type="button"
              onClick={() =>
                printThermalReceipt(document.querySelector(".printable-bill"))
              }
              className="print-btn"
            >
              🖨️ Print Receipt
        ...
        <div className="bill-container printable-bill receipt-card">
          ...
          <div className="bill-details">
            <div className="bill-section">
              <h4>Patient Information</h4>
              <p>
                <strong>Name:</strong> {generatedBill.patientName}
              </p>
              <p>
                <strong>Age:</strong> {generatedBill.patientAge}
              </p>
              <p>
                <strong>Phone:</strong> {generatedBill.patientPhone}
              </p>
              <p>
                <strong>CNIC:</strong> {generatedBill.patientCNIC}
              </p>
            </div>
            ...
            <div className="bill-section">
              ...
              <p>
                <strong>Token:</strong> {generatedBill.token}
              </p>
              <p>
                <strong>Date:</strong>{" "}
                {new Date(generatedBill.collectedAt).toLocaleDateString()}
              </p>
              <p>
                <strong>Time:</strong>{" "}
                {new Date(generatedBill.collectedAt).toLocaleTimeString()}
              </p>
            </div>
              ...
              <div className="bill-breakdown">
                ...
                {generatedBill.medicines?.length > 0 && (
                  <div className="bill-item">
                    ...
                    <span>
                      PKR{" "}
                      {generatedBill.medicines.reduce(
                ...
                <div className="bill-total">
                  ...
                  <span>
                    <strong>PKR {generatedBill.total}</strong>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bill-footer">
              <p>
                <strong>Collected by:</strong> {generatedBill.collectedBy}
              </p>
              <p className="bill-thankyou">
              ...
              {existingBills.map((bill) => (
                <div key={bill._id} className="bill-card">
                  ...
                  <div className="bill-card-details">
                    <p>
                      <strong>Token:</strong> {bill.token}
                    </p>
                    <p>
                      <strong>Total:</strong> PKR {bill.total}
                    </p>
                    <p>
                      <strong>Collected by:</strong> {bill.collectedBy}
                    </p>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setTimeout(
                            () =>
                              printThermalReceipt(
                                document.querySelector(".printable-bill"),
                              ),
                            300,
                          );
                        }}
